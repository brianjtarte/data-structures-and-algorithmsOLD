...
const longestString = (arr) => {
  let longString;
  arr.reduce((strVal, currentVal) => strVal.length > currentVal.length ? longString = strVal : longString = currentVal, '');
...
const findHappiness = (arr) => {
  return arr.filter(happyHappyJoyJoy => happyHappyJoyJoy.includes(':)'));
};
...
const onlyOddChars = (str) => {
  return str.split('').filter((pewpewpew,idx) => idx % 2 === 1).join('');
};
/* ------------------------------------------------------------------------------------------------
CHALLENGE 6 
