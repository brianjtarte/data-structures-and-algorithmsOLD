...
const addTwo = (arr) => {
  const addDos = [];
  arr.forEach(num => addDos.push(num + 2));
...
const containsAnd = (arr) => {
  return arr.filter(strand => strand.includes('and'));
};
...
const oddValues = (arr) => {
  return arr.filter(oddOnly => oddOnly % 2 === 1 );
};
...
const notInFirstArray = (forbiddenValues, arr) => {
  return arr.filter(pineappleBelongsOnPizza => !forbiddenValues.includes(pineappleBelongsOnPizza));
};
...
const getBaseStatGreaterThan = (arr, minBaseStat) => {
  return arr.filter(whatIsAPokemon => whatIsAPokemon.baseStat > minBaseStat);
};
